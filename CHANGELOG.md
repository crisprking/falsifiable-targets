...
## [1.5.0] — 2026-06-02 — Live Open Targets adapter (R3 direction-of-effect evidence)

**Minor release. Additive only — no rule changes. Ruleset SHA unchanged
(`35ef2b2ab5363298...`, RULESET_VERSION 1.2.0).**

### Added

- `adapters/opentargets.py` — `OpenTargetsAdapter`, a protocol-conforming
  adapter that pulls live genetics evidence from the Open Targets Platform
  GraphQL API for the `genetics` section. It **supplies evidence only**
  (genetic direction of effect + provenance + a quality tier); the
  direction/mechanism *concordance verdict* stays in R3's SHA-locked rule
  logic, so audits remain hashable and the adapter stays swappable.
- Direction of effect is derived across three datasources — `gene_burden`
  (strong), ClinVar / `eva` (moderate), and `gwas_credible_sets` + molQTL
  coloc (weak) — emitting `doe_gene_direction`
  (`GoF_increases_risk` | `LoF_increases_risk` | `conflicting` | `unknown`),
  `doe_tier`, `doe_sources`, and `doe_n_assessments`. Disagreement across
  sources is surfaced as `conflicting`, never averaged.
- Backward-compatible keys R3 already reads are preserved unchanged
  (`gwas_hits`, `mendelian_evidence`, `somatic_driver_evidence`,
  `open_targets_score`).
- On-disk response caching (honours `AE_CACHE_DIR` and `AE_OFFLINE=1`) for
  deterministic replay; retry/backoff on HTTP 429/502/503.
- `OpenTargetsAdapter` exported from `adapters/__init__.py`; drops into
  `CompositeAdapter` above `FixtureAdapter`.
- `docs/OPEN_TARGETS_ADAPTER.md` — wiring and design notes.
- Optional dependency group `live` (`requests>=2.28`); also folded into `all`.

### Known limitations (flagged in source)
...
- The three per-datasource directional extractors (`_burden_direction`,
  ...
  against the live Open Targets schema** before relying on the derived
  direction in production. Each returns `None` ("no directional signal")
  until confirmed, so unconfirmed rows degrade safely to `unknown`/`absent`
  rather than producing a wrong call.
- Not wired into `default_composite()` — opt in explicitly so
  offline/deterministic runs remain the default.
